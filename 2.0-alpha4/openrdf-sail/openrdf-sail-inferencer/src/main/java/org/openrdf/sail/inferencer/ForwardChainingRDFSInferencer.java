...
import org.openrdf.sail.helpers.InferencerConnectionWrapper;
...
public abstract class ForwardChainingRDFSInferencer extends SailWrapper {
		...
		throws SailException
	{
		...
		while (hasNewStatements()) {
			iteration++;
			ThreadLog.trace("starting iteration " + iteration);
			_prepareIteration();
			...
			ThreadLog.trace("iteration " + iteration + " done; inferred " + nofInferred + " new statements");
		...
		for (int i = 0; i < RDFSRules.RULECOUNT; i++) {
			ThreadLog.trace("rule " + RDFSRules.RULENAMES[i] + ":\t#inferred=" + _ruleCount[i]);
		}
	...
	protected void _addAxiomStatements(ForwardChainingInferencerConnection con)
		throws SailException
	{
		ThreadLog.trace("Inserting axiom statements");
