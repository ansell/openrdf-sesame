...
//		ThreadLog.trace("Rule " + RDFSRules.RULENAMES[rule] + " inferred " + result + " new triples.");
		...
		throws SailException
	{
		...
		CloseableIterator<MemStatement> iter = _iterateNewStatements(null, null, null);
		...
		throws SailException
	{
		...
		CloseableIterator<MemStatement> ntIter = _iterateNewStatements(null, null, null);
		...
		while (ntIter.hasNext()) {
			...
			CloseableIterator<MemStatement> t1Iter = _iterateAllStatements(aaa, RDFS.DOMAIN, null);
		...
		throws SailException
	{
		...
		CloseableIterator<MemStatement> ntIter = _iterateNewStatements(null, RDFS.DOMAIN, null);
		...
		while (ntIter.hasNext()) {
			...
			if (aaa instanceof URI && zzz instanceof Resource) {
				CloseableIterator<MemStatement> t1Iter = _iterateAllStatements(null, (URI)aaa, null);
		...
		throws SailException
	{
		...
		while (ntIter.hasNext()) {
			...
			if (uuu instanceof Resource) {
				CloseableIterator<MemStatement> t1Iter = _iterateAllStatements(aaa, RDFS.RANGE, null);
		...
		throws SailException
	{
		...
		CloseableIterator<MemStatement> ntIter = _iterateNewStatements(null, RDFS.RANGE, null);
		...
		throws SailException
	{
		...
		CloseableIterator<MemStatement> ntIter = _iterateNewStatements(null, RDFS.SUBPROPERTYOF, null);
		...
		while (ntIter.hasNext()) {
			...
			if (bbb instanceof Resource) {
				CloseableIterator<MemStatement> t1Iter = _iterateAllStatements((Resource)bbb, RDFS.SUBPROPERTYOF, null);
		...
		throws SailException
	{
		...
		while (ntIter.hasNext()) {
			...
			if (ccc instanceof Resource) {
				CloseableIterator<MemStatement> t1Iter = _iterateAllStatements(null, RDFS.SUBPROPERTYOF, bbb);
		...
		throws SailException
	{
		...
		CloseableIterator<MemStatement> iter = _iterateNewStatements(null, RDF.TYPE, RDF.PROPERTY);
		...
		throws SailException
	{
		...
		while (ntIter.hasNext()) {
			...
			CloseableIterator<MemStatement> t1Iter = _iterateAllStatements(aaa, RDFS.SUBPROPERTYOF, null);
		...
		throws SailException
	{
		...
		CloseableIterator<MemStatement> iter = _iterateNewStatements(null, RDF.TYPE, RDFS.CLASS);
		...
		throws SailException
	{
		...
		CloseableIterator<MemStatement> ntIter = _iterateNewStatements(null, RDFS.SUBCLASSOF, null);
		...
		while (ntIter.hasNext()) {
			...
			if (yyy instanceof Resource) {
				CloseableIterator<MemStatement> t1Iter = _iterateAllStatements(null, RDF.TYPE, xxx);
		...
		throws SailException
	{
		...
		CloseableIterator<MemStatement> ntIter = _iterateNewStatements(null, RDF.TYPE, null);
		...
		while (ntIter.hasNext()) {
			...
			if (xxx instanceof Resource) {
				CloseableIterator<MemStatement> t1Iter = _iterateAllStatements((Resource)xxx, RDFS.SUBCLASSOF, null);
		...
		throws SailException
	{
		...
		while (ntIter.hasNext()) {
			...
			if (yyy instanceof Resource) {
				CloseableIterator<MemStatement> t1Iter = _iterateAllStatements((Resource)yyy, RDFS.SUBCLASSOF, null);
		...
		throws SailException
	{
		...
		while (ntIter.hasNext()) {
			...
			if (zzz instanceof Resource) {
				CloseableIterator<MemStatement> t1Iter = _iterateAllStatements(null, RDFS.SUBCLASSOF, yyy);
		...
		throws SailException
	{
		...
		CloseableIterator<MemStatement> iter = _iterateNewStatements(null, RDF.TYPE, RDFS.CONTAINERMEMBERSHIPPROPERTY);
		...
		throws SailException
	{
		...
		CloseableIterator<MemStatement> iter = _iterateNewStatements(null, RDF.TYPE, RDFS.DATATYPE);
	...
	private CloseableIterator<MemStatement> _iterateNewStatements(
		Resource subj, URI pred, Value obj)
	{
		...
		return new MemStatementIterator(_newThisIteration,
				memSubj, memPred, memObj, null,
				false, false, ReadMode.TRANSACTION);
	}

	private CloseableIterator<MemStatement> _iterateAllStatements(
		Resource subj, URI pred, Value obj)
	{
		MemoryStore store = (MemoryStore)getBaseSail();
		return store.createStatementIterator(
				subj, pred, obj, null,
				false, false, ReadMode.TRANSACTION);
	}
		...
		implements SailConnectionListener
	{
			...
			throws SailException
		{
			if (_statementsRemoved) {
				ThreadLog.trace("statements removed, starting inferencing from scratch");
				_clearInferred();
				_addAxiomStatements(this);

				_newStatements.clear();
				CloseableIterator<MemStatement> stIter = _iterateAllStatements(null, null, null);
