 ...
 * MemoryStore.
 */
public class MemStatementIterator extends CloseableIteratorBase<MemStatement> {
	 ...
	 */
	private MemResource _context;

	/**
	 * Flag indicating whether this iterator should only return explicitly
	 * added statements.
	 */
	private boolean _explicitOnly;

	/**
	 * Flag indicating whether this iterator should only return statements from
	 * named contexts (i.e.: exclude the null context).
	 */
	private boolean _namedContextsOnly;

	/**
	 * Flag indicating whether or not the iterator should read any
	 * non-committed changes to the data.
	 */
	...
	public MemStatementIterator(MemStatementList statementList) {
		this(statementList, null, null, null, null,
			false, false, ReadMode.COMMITTED);
	}
	 ...
	 * @param context context of pattern.
	 */
	public MemStatementIterator(MemStatementList statementList,
		MemResource subject, MemURI predicate, MemValue object,
		MemResource context, boolean explicitOnly, boolean namedContextsOnly,
		ReadMode readMode)
	{
		...
		_namedContextsOnly = namedContextsOnly;
		_readMode = readMode;
	...
	private void _findNextStatement() {
		...
		for (; _nextStatementIdx < _statementList.size(); _nextStatementIdx++) {
			...
			if ((_subject == null || _subject == _nextStatement.getSubject()) &&
				...
				(_context == null || _context == _nextStatement.getContextInternal()))
			{
				// A matching statement has been found, check if it should be skipped
				// due to _explicitOnly, _namedContextsOnly and _readMode requirements

				if (_namedContextsOnly &
					_nextStatement.getContextInternal() instanceof MemNullContext)
				{
					// Statement from null context; skip it
					continue;
				}
				...
				if (ReadMode.COMMITTED.equals(_readMode)) {
					...
					if (TxnStatus.DEPRECATED.equals(txnStatus)
							|| TxnStatus.ZOMBIE.equals(txnStatus))
					{
					...
					if (_explicitOnly) {
						if (!_nextStatement.isExplicit()
								&& (! TxnStatus.EXPLICIT.equals(txnStatus))
	...
	public boolean hasNext() {
		if (_nextStatement == null &&
			_statementList != null &&
			_nextStatementIdx < _statementList.size())
		{
			_findNextStatement();
		}
