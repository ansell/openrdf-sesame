...
class NamespaceStore implements Iterable<NamespaceImpl> {
	...
	/**
	 * Map storing namespace information by their namespace name.
	 */
	...
	public void setNamespace(String prefix, String name) {
		NamespaceImpl ns = null;

		if (prefix == null) {
			// Remove any mapping for the specified name
			ns = _namespacesMap.remove(name);
			if (ns != null) {
		...
		ns = _namespacesMap.get(name);

		if (ns != null && ns.getPrefix().equals(prefix)) {
			// prefix is already associated with this namespace, ignore
			return;
		}

		// Remove any existing mapping for the specified prefix
		Iterator<NamespaceImpl> iter = _namespacesMap.values().iterator();
		while (iter.hasNext()) {
			NamespaceImpl otherNS = iter.next();

			if (otherNS.getPrefix().equals(prefix)) {
				iter.remove();
				break;
			}
		}
		...
		if (ns != null) {
			...
			ns = new NamespaceImpl(prefix, name);
