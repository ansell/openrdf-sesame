...
public class Rio {
	...
	private static void _registerParser(RDFFormat rdfFormat, String className) {
		final Logger logger = LoggerFactory.getLogger(Rio.class); 

		try {
			@SuppressWarnings("unchecked")
			Class<? extends RDFParser> parserClass = (Class<? extends RDFParser>)Class.forName(className);
			_parserFactory.addType(rdfFormat, parserClass);
		}
		catch (ClassNotFoundException e) {
			logger.info("Unable to load RDF parser class: " + className, e);
		}
		catch (SecurityException e) {
			logger.warn("Not allowed to load RDF parser class: " + className, e);
		}
		catch (ClassCastException e) {
			logger.error("Parser class does not implement RDFParser interface: " + className, e);
		}
		catch (IllegalArgumentException e) {
			logger.error(e.getMessage(), e);
		}
		catch (RuntimeException e) {
			logger.error("Unexpected error while trying to register RDF parser", e);
		}
	}
	...
	private static void _registerWriter(RDFFormat rdfFormat, String className) {
		...
		try {
			@SuppressWarnings("unchecked")
			Class<? extends RDFWriter> writerClass = (Class<? extends RDFWriter>)Class.forName(className);
			_writerFactory.addType(rdfFormat, writerClass);
		}
		catch (ClassNotFoundException e) {
			logger.info("Unable to load RDF writer class: " + className, e);
		}
		catch (SecurityException e) {
			logger.warn("Not allowed to load RDF writer class: " + className, e);
		}
		catch (ClassCastException e) {
			logger.error("Parser class does not implement RDFWriter interface: " + className, e);
		}
		catch (IllegalArgumentException e) {
			logger.error(e.getMessage(), e);
		}
		catch (RuntimeException e) {
			logger.error("Unexpected error while trying to register RDF writer", e);
		}
	}
