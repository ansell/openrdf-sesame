...
public class BinaryQueryResultWriter implements TupleQueryResultWriter {
	...
	private CharsetEncoder _charsetEncoder = Charset.forName("UTF-8").newEncoder();
	...
	private void _writeString(String s)
		throws IOException
	{
		ByteBuffer byteBuf = _charsetEncoder.encode(CharBuffer.wrap(s));
		_out.writeInt(byteBuf.remaining());
		_out.write(byteBuf.array(), 0, byteBuf.remaining());
	}
}
