...
import static org.openrdf.query.resultio.sparqlxml.SPARQLResultsXMLConstants.RESULT_SET_DISTINCT_ATT;
import static org.openrdf.query.resultio.sparqlxml.SPARQLResultsXMLConstants.RESULT_SET_ORDERED_ATT;
	...
	private class SPARQLResultsParser extends SimpleSAXAdapter {
			...
			throws SAXException
		{
			if (BINDING_TAG.equals(tagName)) {
				...
				boolean ordered = Boolean.parseBoolean(atts.get(RESULT_SET_ORDERED_ATT));
				boolean distinct = Boolean.parseBoolean(atts.get(RESULT_SET_DISTINCT_ATT));

				try {
					_handler.startQueryResult(_bindingNames, distinct, ordered);
				}
				catch (TupleQueryResultHandlerException e) {
					throw new SAXException(e);
				}
