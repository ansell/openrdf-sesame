...
class ConstructorBuilder {
	...
	public TupleExpr buildConstructor(TupleExpr bodyExpr, TupleExpr constructExpr) {
		return buildConstructor(bodyExpr, constructExpr, true);
	}

	public TupleExpr buildConstructor(TupleExpr bodyExpr) {
		return buildConstructor(bodyExpr, bodyExpr, false);
	}
	...
	private TupleExpr buildConstructor(TupleExpr bodyExpr, TupleExpr constructExpr, boolean explicit) {
		...
		if (explicit) {
			...
			for (StatementPattern sp : spCollector.getStatementPatterns()) {
				createExtensionElem(sp.getSubjectVar(), extElemMap);
				createExtensionElem(sp.getPredicateVar(), extElemMap);
				createExtensionElem(sp.getObjectVar(), extElemMap);
			}
		...
		for (StatementPattern sp : spCollector.getStatementPatterns()) {
			projList.add(createProjection(tupleExpr, sp));
		}
	 ...
	 * @param extElemMap
	 */
	private void createExtensionElem(Var var, Map<Var, ExtensionElem> extElemMap) {
	...
	private Projection createProjection(TupleExpr bodyExpr, StatementPattern sp) {
