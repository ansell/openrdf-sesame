...
public class BTree {
	...
	private BTreeValueComparator comparator;
	...
	private LinkedList<Node> nodesInUse;

	/**
	 * List containing the most recently used nodes but that are no longer used
	 * anymore.
	 */
	private LinkedList<Node> mruNodes;
		...
		throws IOException
	{
		...
		nodesInUse = new LinkedList<Node>();
		mruNodes = new LinkedList<Node>();
		...
		if (fileChannel.size() == 0L) {
			...
			maxNodeID = offset2nodeID(fileChannel.size() - nodeSize);
		}
