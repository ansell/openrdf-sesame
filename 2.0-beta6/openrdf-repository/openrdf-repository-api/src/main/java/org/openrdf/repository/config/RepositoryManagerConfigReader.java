	...
	private static class ServerConfigParser extends SimpleSAXAdapter {
		...
		public void startTag(String tagName, Map<String, String> atts, String text) {
			if (tagName.equals("admin")) {
				...
				// Set class name or repository, might be null but that's OK
				_currentRepository.setClassName(atts.get("class"));
			}
		...
		private void addCurrentUser() {
			...
			if (correctConfig) {
				logger.debug("user {} was parsed correctly", login);
		...
		private void addCurrentRepository() {
			boolean correctConfig = true;
			...
			if (_currentRepository.getSailConfigStack().isEmpty() && _currentRepository.getClassName() == null) {
				logger.error("no sail defined for repository " + id);
				correctConfig = false;
			}

			if (correctConfig) {
				logger.debug("repository {} was parsed correctly", id);
