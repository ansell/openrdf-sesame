...
public class RDFXMLParserTest {
		...
		throws Exception
	{
		...
		// Add W3C's manifest
		URL w3cManifest = _resolveURL(W3C_MANIFEST_FILE);
		con.add(w3cManifest, W3C_MANIFEST_FILE, RDFFormat.RDFXML);

		// Add our own manifest
		URL localManifest = _resolveURL(OPENRDF_MANIFEST_FILE);
	...
	private static URL _resolveURL(String urlString)
	...
	private static class PositiveParserTest extends TestCase {
			...
			throws Exception
		{
			...
			InputStream in = _resolveURL(_inputURL).openStream();
			rdfxmlParser.parse(in, _inputURL);
			in.close();
			...
			in = _resolveURL(_outputURL).openStream();
			ntriplesParser.parse(in, _inputURL);
			in.close();
