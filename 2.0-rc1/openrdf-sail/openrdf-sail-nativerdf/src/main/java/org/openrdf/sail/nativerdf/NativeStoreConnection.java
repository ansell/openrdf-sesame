		...
		throws SailException
	{
		...
		try {
			...
			for (int contextID : contextIDs) {
				BTreeIterator iter = nativeStore.getTripleStore().getTriples(-1, -1, -1, contextID, false,
						transactionActive());
				try {
		...
		try {
			...
			for (int i = 0; i < contextIDList.size(); i++) {
				...
				if (hasConnectionListeners()) {
					...
					// be reported
					BTreeIterator btreeIter = tripleStore.getTriples(subjID, predID, objID,
							NativeValue.UNKNOWN_ID, explicit);
