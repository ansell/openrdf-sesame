...
public class Console {
	...
	private static final AppVersion VERSION = new AppVersion(2, 0, "beta6");
		...
		throws IOException
	{
		try {
			...
			writeError("Not allowed to access the server");
		}
		catch (IOException e) {
		...
		RepositoryException
	{
		...
		try {
			...
			long endTime = System.currentTimeMillis();
			writeln("Query evaluated in " + (endTime - startTime) + " ms");
		}
		finally {
			con.close();
		}
	...
	private void writeParseError(RDFParseException e) {
