...
public abstract class Protocol {
	...
	public static final String getNamespacePrefixLocation(String repositoryLocation, String prefix) {
		return repositoryLocation + "/" + prefix;
	}
	...
	public static byte[] encodeExceptionInfo(ExceptionInfo exInfo)
		throws IOException
	{
		...
		infoProperties.setProperty(ExceptionInfo.EXCEPTION_CLASS_KEY, exInfo.getExceptionClassName());
		infoProperties.setProperty(ExceptionInfo.EXCEPTION_MESSAGE_KEY, exInfo.getExceptionMessage());
		infoProperties.setProperty(ExceptionInfo.HAS_PARSE_INFO_KEY, Boolean.toString(exInfo.hasParseInfo()));
		if (exInfo.hasParseInfo()) {
			ParseInfo parseInfo = exInfo.getParseInfo();
			if (parseInfo.getEncounteredToken() != null) {
				infoProperties.setProperty(ParseInfo.ENCOUNTERD_TOKEN_KEY, parseInfo.getEncounteredToken());
			}
			infoProperties.setProperty(ParseInfo.LINE_KEY, Long.toString(parseInfo.getLineNumber()));
			infoProperties.setProperty(ParseInfo.COLUMN_KEY, Long.toString(parseInfo.getColumnNumber()));
			if (parseInfo.getExpectedTokens() != null) {
				infoProperties.setProperty(ParseInfo.EXPECTED_TOKEN_COUNT_KEY,
						Integer.toString(parseInfo.getExpectedTokens().size()));
				int expectedTokenCount = 0;
				Iterator<String> expectedTokenIterator = parseInfo.getExpectedTokens().iterator();
				while (expectedTokenIterator.hasNext()) {
					infoProperties.setProperty(ParseInfo.EXPECTED_TOKEN_KEY_PREFIX + expectedTokenCount,
							expectedTokenIterator.next());
					expectedTokenCount++;
				}
			}
		}

		infoProperties.store(result, null);
		result.flush();

		return result.toByteArray();
	}

	public static ExceptionInfo decodeExceptionInfo(byte[] exInfoBytes)
		throws IOException
	{
		ExceptionInfo result = null;

		ByteArrayInputStream in = new ByteArrayInputStream(exInfoBytes);
		Properties infoProperties = new Properties();
		infoProperties.load(in);

		String className = infoProperties.getProperty(ExceptionInfo.EXCEPTION_CLASS_KEY);
		String message = infoProperties.getProperty(ExceptionInfo.EXCEPTION_MESSAGE_KEY);
		result = new ExceptionInfoBase(className, message);
		boolean hasParseInfo = Boolean.parseBoolean(infoProperties.getProperty(ExceptionInfo.HAS_PARSE_INFO_KEY));
		if (hasParseInfo) {
			String encounteredToken = infoProperties.getProperty(ParseInfo.ENCOUNTERD_TOKEN_KEY);
			long line = Long.parseLong(infoProperties.getProperty(ParseInfo.LINE_KEY));
			long position = Long.parseLong(infoProperties.getProperty(ParseInfo.COLUMN_KEY));
			Set<String> expectedTokens = null;
			String expectedTokenCountString = infoProperties.getProperty(ParseInfo.EXPECTED_TOKEN_COUNT_KEY);
			if (expectedTokenCountString != null) {
				int expectedTokensCount = Integer.parseInt(expectedTokenCountString);
				expectedTokens = new HashSet<String>(expectedTokensCount);
				for (int expectedTokensIndex = 0; expectedTokensIndex < expectedTokensCount; expectedTokensIndex++)
				{
					expectedTokens.add(infoProperties.getProperty(ParseInfo.EXPECTED_TOKEN_KEY_PREFIX
							+ expectedTokensIndex));
				}
			}
			else {
				expectedTokens = Collections.emptySet();
			}
			ParseInfo parseInfo = new ParseInfoBase(encounteredToken, line, position, expectedTokens);
