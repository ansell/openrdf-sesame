...
public class StatementsController extends AbstractController {
		...
		throws Exception
	{
		...
		if ("GET".equals(reqMethod)) {
			...
			HeaderElement contentType = HeaderElement.parse(request.getContentType());
		...
		throws IOException, ClientRequestException, InternalServerException
	{
		...
		try {
			...
			throw new ClientRequestException(SC_BAD_REQUEST, "Malformed transaction data: " + e.getMessage(), e,
					exInfo);
		}
		...
		throws IOException, ClientRequestException, InternalServerException
	{
		...
		if (rdfFormat == null) {
			throw new ClientRequestException(SC_UNSUPPORTED_MEDIA_TYPE, "Unsupported content type: " + mimeType);
		}
