...
public class EvaluationStrategyImpl implements EvaluationStrategy {
	...
	public CloseableIteration<BindingSet, QueryEvaluationException> evaluate(Order node, BindingSet bindings)
		throws QueryEvaluationException
	{
		throw new QueryEvaluationException("Query result ordering not yet supported");
	}
		...
		throws ValueExprEvaluationException, QueryEvaluationException
	{
		...
		if (QueryEvaluationUtil.isSimpleLiteral(val)) {
			String text = ((Literal)val).getLabel();
			boolean result = node.getOpPattern().matcher(text).find();
		...
		throws ValueExprEvaluationException, QueryEvaluationException
	{
		...
		return BooleanLiteralImpl.valueOf(compare(leftVal, rightVal, node.getOperator()));
	}
		...
		throws ValueExprEvaluationException, QueryEvaluationException
	{
		...
		try {
			while (result == false && iter.hasNext()) {
				...
				try {
					result = compare(leftValue, rightValue, node.getOperator());
				}
				catch (ValueExprEvaluationException e) {
	...
	private static boolean compare(Value leftVal, Value rightVal, CompareOp operator)
	...
	private static boolean compareLiterals(Literal leftLit, Literal rightLit, CompareOp operator)
		throws ValueExprEvaluationException
	{
		// type precendence:
		// - simple literal
