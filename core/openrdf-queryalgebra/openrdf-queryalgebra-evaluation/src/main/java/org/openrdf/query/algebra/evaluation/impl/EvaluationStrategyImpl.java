 ...
 * @author David Huynh
 */
public class EvaluationStrategyImpl implements QueryModelVisitor<QueryEvaluationException>, EvaluationStrategy {

	final Logger logger = LoggerFactory.getLogger(this.getClass()); 
	
	private static final BindingSet EMPTY_BINDING_SET = new QueryBindingSet(0);
				...
				&& (rightLit.getLanguage() == null && (rightDatatype == null || rightDatatype.equals(XMLSchema.STRING))))
		{
			...
			if (USE_TYPED_LITERALS) {
				if (commonDatatype.equals(XMLSchema.DOUBLE)) {
					...
					// Note: XMLGregorianCalendar.compare() returns compatible values
	...
	public void meet(final Difference difference) {
		...
		Iteration<BindingSet, QueryEvaluationException> leftArg = new DelayedIteration<BindingSet, QueryEvaluationException>() {

			protected Iteration<BindingSet, QueryEvaluationException> createIterator() throws QueryEvaluationException {
				return evaluate(difference.getLeftArg(), bindings);
			}
		};

		Iteration<BindingSet, QueryEvaluationException> rightArg = new DelayedIteration<BindingSet, QueryEvaluationException>() {
	...
	public void meet(Distinct distinct) throws QueryEvaluationException {
		_evaluate = new DistinctIteration<BindingSet, QueryEvaluationException>(evaluate(distinct.getArg(), _bindings));
	}
	...
	public void meet(Extension extension) throws QueryEvaluationException {
	...
	public void meet(Join join) throws QueryEvaluationException {
	...
	public void meet(Null node) throws QueryEvaluationException {
		_value = null;
	}

	public void meet(OptionalJoin optionalJoin) throws QueryEvaluationException {
	...
	public void meet(Projection projection) throws QueryEvaluationException {
		_evaluate = new ProjectionIterator(this.evaluate(projection.getArg(), _bindings), projection, _bindings);
	}

	public void meet(ProjectionElem node) throws QueryEvaluationException {
	}

	public void meet(RowSelection rowSelection) throws QueryEvaluationException {
		CloseableIteration<BindingSet, QueryEvaluationException> result = evaluate(rowSelection.getArg(), _bindings);
	...
	public void meet(Selection selection) throws QueryEvaluationException {
		_evaluate = new SelectionIterator(this, selection, _bindings);
	}

	public void meet(SingletonSet singletonSet) {
		_evaluate = new SingletonIteration<BindingSet, QueryEvaluationException>(EMPTY_BINDING_SET);
	}

	public void meet(StatementPattern sp) throws QueryEvaluationException {
		final BindingSet bindings = _bindings;
		...
		try {
			if (sp.getScope() == Scope.ALL_CONTEXTS) {
				...
				if (contextValue == null) {
					...
					stIter = new FilterIteration<Statement, QueryEvaluationException>(_tripleSource.getStatements((Resource)subjValue,
							(URI)predValue, objValue))
					{
	...
	public void meet(DatatypeFunc node) throws QueryEvaluationException {
	...
	public void meet(StrFunc node) throws QueryEvaluationException {
	...
	public void meet(LabelFunc node) throws QueryEvaluationException {
		Value value = getValue(node.getArg(), _bindings);
	...
	public void meet(LangFunc node) throws QueryEvaluationException {
		Value value = getValue(node.getArg(), _bindings);
	...
	public void meet(LocalNameFunc node) throws QueryEvaluationException {
		Value value = getValue(node.getArg(), _bindings);
	...
	public void meet(NamespaceFunc node) throws QueryEvaluationException {
		Value value = getValue(node.getArg(), _bindings);
	...
	private Value _getVarValue(Var var, BindingSet bindings) throws QueryEvaluationException {
		if (var == null) {
			return null;
		}
		else {
