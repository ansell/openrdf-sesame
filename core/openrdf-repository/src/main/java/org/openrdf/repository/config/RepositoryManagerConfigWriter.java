...
public class RepositoryManagerConfigWriter {
		...
		throws IOException
	{
		...
		_xmlWriter.comment("server parameters");
		_writeAdmin();
		_writeLog();
		_writeTmp();
	...
	private void _writeHeader()
		throws IOException
	{
		_xmlWriter.startTag("server-conf");
	}

	private void _writeFooter()
		throws IOException
	{
		_xmlWriter.endTag("server-conf");
	}
	...
	private void _writeLog()
		throws IOException
	{
		String logDir = _config.getLogDir();
		if (logDir == null) {
			logDir = "";
		}
		_xmlWriter.setAttribute("dir", logDir);
		_xmlWriter.setAttribute("level", _config.getLogLevel());
		_xmlWriter.emptyElement("log");
	}

	private void _writeTmp()
		throws IOException
	{
		String tmpDir = _config.getTmpDir();
		if (tmpDir == null) {
			tmpDir = "";
		}
		_xmlWriter.setAttribute("dir", tmpDir);
		_xmlWriter.emptyElement("tmp");
	}
		...
		throws IOException
	{
		...
		_xmlWriter.setAttribute("worldWriteable", repConfig.isWorldWriteable());
