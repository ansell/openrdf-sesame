...
public abstract class SailConnectionBase implements SailConnection {
	 ...
	 * on the connection.
	 */
	private MultiReadSingleWriteLockManager connectionLockManager = new MultiReadSingleWriteLockManager();
	 ...
	 */
	private ExclusiveLockManager txnLockManager = new ExclusiveLockManager();
		...
		try {
			if (isOpen) {
				try {
					...
					// there should now be no more active iterations.
					...
					if (txnActive) {
						...
						try {
							...
							// rollback method will
							// try to obtain a thread lock.
							rollbackInternal();
						}
						finally {
							txnActive = false;
