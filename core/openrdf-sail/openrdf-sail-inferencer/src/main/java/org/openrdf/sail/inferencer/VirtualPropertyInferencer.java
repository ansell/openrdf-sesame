...
public class VirtualPropertyInferencer extends SailWrapper {
	...
	private static final GraphQuery DIRECT_SUBCLASSOF_MATCHER;

	private static final GraphQuery DIRECT_SUBCLASSOF_QUERY;

	private static final GraphQuery DIRECT_SUBPROPERTYOF_MATCHER;

	private static final GraphQuery DIRECT_SUBPROPERTYOF_QUERY;

	private static final GraphQuery DIRECT_TYPE_MATCHER;

	private static final GraphQuery DIRECT_TYPE_QUERY;

	static {
		try {
			DIRECT_SUBCLASSOF_MATCHER = QueryParserUtil.parseGraphQuery(QueryLanguage.SERQL,
					"CONSTRUCT * FROM {X} sesame:directSubClassOf {Y} ");

			DIRECT_SUBPROPERTYOF_MATCHER = QueryParserUtil.parseGraphQuery(QueryLanguage.SERQL,
					"CONSTRUCT * FROM {X} sesame:directType {Y}");

			DIRECT_TYPE_MATCHER = QueryParserUtil.parseGraphQuery(QueryLanguage.SERQL,
					"CONSTRUCT * FROM {X} sesame:directSubPropertyOf {Y}");
			...
			DIRECT_SUBCLASSOF_QUERY = QueryParserUtil.parseGraphQuery(QueryLanguage.SERQL,
					"CONSTRUCT {X} sesame:directSubClassOf {Y} " + "FROM {X} rdfs:subClassOf {Y} "
							...
							+ "FROM {X} rdfs:subClassOf {Z} rdfs:subClassOf {Y} " + "WHERE X != Z AND Z != Y)");
			...
			DIRECT_SUBPROPERTYOF_QUERY = QueryParserUtil.parseGraphQuery(QueryLanguage.SERQL,
					"CONSTRUCT {X} sesame:directSubPropertyOf {Y} " + "FROM {X} rdfs:subPropertyOf {Y} "
							+ "WHERE X != Y AND NOT EXISTS (" + "SELECT Z "
							+ "FROM {X} rdfs:subPropertyOf {Z} rdfs:subPropertyOf {Y} " + "WHERE X != Z AND Z != Y)");
			...
			DIRECT_TYPE_QUERY = QueryParserUtil.parseGraphQuery(QueryLanguage.SERQL,
					"CONSTRUCT {X} sesame:directType {Y} " + "FROM {X} rdf:type {Y} " + "WHERE NOT EXISTS ("
							+ "SELECT Z " + "FROM {X} rdf:type {Z} rdfs:subClassOf {Y} " + "WHERE Z != Y)");
		}
	...
	private class VirtualPropertyInferencerConnection extends SailConnectionWrapper {
			...
			throws SailException
		{
			...
			try {
				...
				ThreadLog.trace("existing virtual properties: " + oldStatements.size());
				ThreadLog.trace("new virtual properties: " + newStatements.size());
				...
				ThreadLog.trace("virtual properties to remove: " + oldStatements.size());
				ThreadLog.trace("virtual properties to add: " + newStatements.size());
				...
				if (!oldStatements.isEmpty() || !newStatements.isEmpty()) {
					...
					try {
						for (Statement st : oldStatements) {
							con.removeInferredStatement(st.getSubject(), st.getPredicate(), st.getObject(), null);
						}

						for (Statement st : newStatements) {
							con.addInferredStatement(st.getSubject(), st.getPredicate(), st.getObject(), null);
						}
		...
		private void evaluateIntoStatements(GraphQuery query, StatementCollector collector)
			throws RDFHandlerException
		{
			collector.startRDF();
			
			GraphQueryResult graphQueryResult = evaluate(query, true);
			try {
				for (Statement st : graphQueryResult) {
