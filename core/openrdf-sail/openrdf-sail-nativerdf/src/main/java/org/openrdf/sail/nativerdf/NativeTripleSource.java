...
class NativeTripleSource implements TripleSource {
		...
		try {
			...
			if (contexts.length > 0) {
				...
				for (Resource context : contexts) {
					...
					perContextIterList.add(new NativeStatementIterator(_nativeStore._tripleStore,
							_nativeStore._valueStore, _nativeStore._valueStore, subj, pred, obj, context, subjID,
				...
				return new NativeStatementIterator(_nativeStore._tripleStore, _nativeStore._valueStore,
						_nativeStore._valueStore, subj, pred, obj, null, subjID, predID, objID, contextID);
			}
		}
		catch (IOException e) {
	...
	{
		return new ExceptionConvertingIteration<Statement, QueryEvaluationException>(_getStatements(subj,
				pred, obj, contexts))
		{
	...
	public CloseableIteration<? extends Statement, SailException> getStatementsSailException(Resource subj,
			URI pred, Value obj, Resource... contexts)
	{
		return new ExceptionConvertingIteration<Statement, SailException>(_getStatements(subj,
				pred, obj, contexts))
		{
