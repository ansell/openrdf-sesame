...
class NativeTripleSource implements TripleSource {
	 ...
	 */
	private CloseableIteration<? extends Statement, IOException> getStatementsInternal(Resource subj,
			URI pred, Value obj, Resource... contexts)
	{
		...
		try {
			...
			if (subj != null) {
				subjID = nativeStore.valueStore.getID(subj);
			...
			if (pred != null) {
				predID = nativeStore.valueStore.getID(pred);
			...
			if (obj != null) {
				objID = nativeStore.valueStore.getID(obj);
			...
			if (contexts.length > 0) {
				...
				for (Resource context : contexts) {
					...
					if (context == null) {
						...
						contextID = nativeStore.valueStore.getID(context);
					...
					perContextIterList.add(new NativeStatementIterator(nativeStore.tripleStore,
							nativeStore.valueStore, nativeStore.valueStore, subj, pred, obj, context, subjID,
							predID, objID, contextID, includeInferred));
				}
				...
				return new NativeStatementIterator(nativeStore.tripleStore, nativeStore.valueStore,
						nativeStore.valueStore, subj, pred, obj, null, subjID, predID, objID,
