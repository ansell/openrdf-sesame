		...
		throws ValueExprEvaluationException
	{
		...
		if (leftArg instanceof Literal && rightArg instanceof Literal) {
					...
					&& (rightDt == null || XMLSchema.STRING.equals(rightDt)))
			{
				...
				String subBefore = "";
				if (index > -1) {
					subBefore = lexicalValue.substring(0, index);
				}
				return valueFactory.createLiteral(subBefore);
			}
			else {
				throw new ValueExprEvaluationException("incompatible operands: " + leftArg + ", " + rightArg);
			}
		}
		else {
			throw new ValueExprEvaluationException("incompatible operands: " + leftArg + ", " + rightArg);
