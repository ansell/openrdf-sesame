...
public class Token {
	 ...
	 * true only if this token is also a regular token. Otherwise, see below for
	 * a description of the contents of this field.
	 */
	public Token next;
	 ...
	 * there are no such special tokens, this field is set to null. When there
	 * are more than one such special token, this field refers to the last of
	 * these special tokens, which in turn refers to the next previous special
	 * token through its specialToken field, and so on until the first special
	 * token (whose specialToken field is null). The next fields of special
	 * tokens refer to other special tokens that immediately follow it (without
	 * an intervening regular token). If there is no such token, this field is
	 * null.
	 */
	 ...
	 * return new IDToken(); to the following switch statement. Then you can cast
