 ...
 * 
 * more documetnation see {@link LuceneSail}
 */
public class LuceneIndex {
	 ...
	 * This consists of the Resource identifier (URI or BNodeID) and the Context ID (the format is "resourceId|contextId") 
	 */
	...
	/**
	 * keep a lit of old monitors that are still iterating but not closed
	 * (open iterators), will be all closed on shutdown
	 * items are removed from list by ReaderMnitor.endReading()
	 * when closing
	 */
	...
	/**
	 * Current monitor holds instance of IndexReader and IndexSearcher
	 * It is used to keep track of readers
	 */
		...
		throws IOException
	{
		...
		if(oldmonitors.size()>0){
			logger.warn("LuceneSail: On shutdown {} IndexReaders were not closed. This is due to non-closed Query Iterators, which must be closed!", oldmonitors.size());
		}
	...
	/**
	 * Returns whether the provided literal is accepted by the LuceneIndex to be indexed. It
	 * for instance does not make much since to index xsd:float.
	 * 
	...
	private String getContextID(Resource resource) {
		if (resource == null)
			return CONTEXT_NULL;
		else return getResourceID(resource);
	}
	...
	/**
	 * Returns a list of Documents representing the specified Resource (empty when no
	 * such Document exists yet). Each document represent a set of statements with the specified Resource as a subject, 
	 * which are stored in a specific context
	 */
	...
	/**
	 * Returns a Document representing the specified Resource & Context combination, or null when no
	 * such Document exists yet.
	 */
	...
	private boolean isPropertyField(String fieldName) {
		return !ID_FIELD_NAME.equals(fieldName) 
				&& !URI_FIELD_NAME.equals(fieldName)
				&& !TEXT_FIELD_NAME.equals(fieldName)
	...
	/**
	 * invalidate readers, free them if possible
	 * (readers that are still open by a {@link LuceneQueryConnection} will not
	 * be closed.
	 * Synchronized on oldmonitors because it manipulates them 
	 * @throws IOException
	 */
		...
		throws IOException
	{
		synchronized (oldmonitors) {
			...
			if(currentMonitor != null)
				// we do NOT close it directly as it may be used by an open result iterator, hence moving it to the 
	...
	private void logIndexStats() {
		try {
			...
			try {
				...
				logger.info("Total documents in the index: "+reader.numDocs() + ", number of deletable documents in the index: "+reader.numDeletedDocs() + ", valid documents: " + count + ", total fields in all documents: " + totalFields + ", average number of fields per document: " + ((double)totalFields)/reader.numDocs());
		...
		throws IOException
	{
		...
		if (document != null) {
			...
			if (hasProperty(fieldName, text, document)) {
				...
				if (nrProperties == 0) {
					logger.info("encountered document with zero properties, should have been deleted: {}", resourceId);
				}
...
//	/**
//	 * Parses an id-string used for a context filed (a serialized resource) back to a resource.
//	 * <b>CAN RETURN NULL</b>
//	 * Inverse method of {@link #getResourceID(Resource)}
//	 * @param idString
//	 * @return null if the passed idString was the {@link #CONTEXT_NULL} constant
//	 */
...
//	}
           ...
           throws ParseException, IOException
   {
           ...
           // Now this is ok
//           if(hits.totalHits > 1)
//                  logger.info("More than one Lucene doc was found with {} == {}", ID_FIELD_NAME, getID(resource));
   ...
   /**
	...
	private QueryParser getQueryParser(URI propertyURI) {
		...
		if(propertyURI == null)
			// if we have no property given, we create a default query parser which has the TEXT_FIELD_NAME as the default field
	 ...
	 */
	public synchronized void addRemoveStatements(Collection<Statement> added, Collection<Statement> removed) throws Exception {
		// Buffer per resource
...
//					HashSet<Resource> contextsToAdd = new HashSet<Resource>();
					...
					if (stmtsToRemove.containsKey(contextId))
						logger.info("Statements are marked to be removed that should not be in the store, for resource {} and context {}. Nothing done.", resource, contextId);
				}
				else {
					// update the Document 
						...
						if (removedStatements != null && !removedStatements.isEmpty())
						{
							...
							for (Statement r : removedStatements) {
								if (r.getObject() instanceof Literal) {
									// remove value from both property field and the corresponding text field
					...
					// keep the predicate/value pairs to ensure that the statement cannot be added twice
					...
					// add all statements to this document, except for those which are already there 
					{
					...
					// update the index with the cloned document, if it contains any meaningful non-system properties
	 ...
	 */
	public synchronized void clearContexts(Resource[] contexts, Sail sail) throws IOException, SailException {
...
//							Resource otherContextOfDocument = getContextResource(f.stringValue()); // can return null
...
//				CloseableIteration<? extends Statement, SailException> it = con.getStatements(resource, null, null, false);
//				while (it.hasNext()) {
//					Statement s = it.next();
//					toAdd.add(s);
//				}
...
//		}
	 ...
	 * @throws IOException 
	 */
	public synchronized void addDocuments(Resource subject, List<Statement> statements) throws IOException {
