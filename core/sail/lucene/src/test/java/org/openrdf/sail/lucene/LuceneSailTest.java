...
//		connection.setAutoCommit(false);
	...
	/**
	 * This test simulates possible flow of calls to the LuceneIndex.
	 * It assert does InexReader and IndexSearcher are not closed while iterating but are finally close.
	 * @throws Exception
	 */
	public void testClosingIndexReaderAndSearcherAllCases() throws Exception{
		...
		String queryString = "SELECT Resource FROM {Resource} <" + MATCHES + "> {}  <" + QUERY + "> {\"sfourponecone\"} ";
		TupleQuery query = connection.prepareTupleQuery(QueryLanguage.SERQL, queryString);
		...
		queryString = "SELECT Resource FROM {Resource} <" + MATCHES + "> {}  <" + QUERY + "> {\"sfourponecone\"} ";
	...
	/**
	 * we experienced problems with the NULL context and lucenesail
	 * in August 2008
	 * @throws Exception
	 */
