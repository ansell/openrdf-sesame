...
import org.openrdf.sail.inferencer.VirtualPropertyInferencer;
...
public class SeRQLQueryTest extends TestCase {
	...
	private List<String> _graphNames;

	private String _entailment;
	...
	protected void runTest()
		throws Exception
	{
		String query = _readQuery();

		// Create a repository with the query data

		Sail sail = new MemoryStore();

		if ("RDF".equals(_entailment)) {
			// do not add inferencers
		}
		else if ("RDFS".equals(_entailment)) {
			sail = new MemoryStoreRDFSInferencer(sail);
		}
		else if ("RDFS-VP".equals(_entailment)) {
			sail = new MemoryStoreRDFSInferencer(sail);
			sail = new VirtualPropertyInferencer(sail);
		}
		else {
			sail.shutDown();
			fail("Invalid value for entailment level:" + _entailment);
		}
		...
		// Add unnamed gaph
		dataCon.add(new URL(_dataFile), null, RDFFormat.forFileName(_dataFile));

		// add named graphs
		for (String graphName : _graphNames) {
		...
		Collection<Statement> actualStatements = Iterations.addAll(result,
		...
		erCon.add(new URL(_resultFile), null, RDFFormat.forFileName(_resultFile));
	...
	private String _readQuery()
		throws IOException
	{
		InputStream stream = new URL(_queryFile).openStream();
		try {
