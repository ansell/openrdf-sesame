...
public class InferencingTest extends TestCase {
	...
	protected Sail _sailStack;

	protected String _inputData;

	protected String _outputData;

	protected boolean _isPositiveTest;
			...
			boolean isPositiveTest)
	{
		...
		_name = name.substring(0, slashLoc) + "-" + name.substring(slashLoc + 1);

		_sailStack = sailStack;
		_inputData = inputData;
		_outputData = outputData;
		_isPositiveTest = isPositiveTest;
	}
		...
		throws Exception
	{
		...
		Repository repository = new SailRepository(_sailStack);
		...
		// Upload input data
		InputStream stream = getClass().getResourceAsStream(_inputData);
		try {
			con.add(stream, _inputData, RDFFormat.NTRIPLES);
			con.commit();
		...
		stream = getClass().getResourceAsStream(_outputData);
		try {
			con.add(stream, _outputData, RDFFormat.NTRIPLES);
			con.commit();
		...
		if (_isPositiveTest && !outputEntailed) {
			File dumpFile = _dumpStatements(RepositoryUtil.difference(expectedStatements, entailedStatements));

			fail("Incomplete entailment, difference between expected and entailed dumped to file " + dumpFile);
		}
		else if (!_isPositiveTest && outputEntailed) {
			File dumpFile = _dumpStatements(expectedStatements);
			fail("Erroneous entailment, unexpected statements dumped to file " + dumpFile);
		}
	}

	private File _dumpStatements(Collection<? extends Statement> statements)
		throws Exception
	{
		// Dump results to tmp file for debugging
		String tmpDir = System.getProperty("java.io.tmpdir");
		File tmpFile = new File(tmpDir, "junit-" + _name + ".nt");
	...
	public static Test suite(Sail sailStack) {
		...
		suite.addTest(_createTestCase(sailStack, "subclassof", "test001", true));
		suite.addTest(_createTestCase(sailStack, "subclassof", "test002", true));
		suite.addTest(_createTestCase(sailStack, "subclassof", "test003", true));
		suite.addTest(_createTestCase(sailStack, "subclassof", "error001", false));
		suite.addTest(_createTestCase(sailStack, "subpropertyof", "test001", true));
		suite.addTest(_createTestCase(sailStack, "subpropertyof", "test002", true));
		suite.addTest(_createTestCase(sailStack, "subpropertyof", "test003", true));
		suite.addTest(_createTestCase(sailStack, "subpropertyof", "error001", false));
		suite.addTest(_createTestCase(sailStack, "subpropertyof", "error002", false));
		suite.addTest(_createTestCase(sailStack, "subpropertyof", "error003", false));
		suite.addTest(_createTestCase(sailStack, "domain", "test001", true));
		suite.addTest(_createTestCase(sailStack, "domain", "error001", false));
		suite.addTest(_createTestCase(sailStack, "range", "test001", true));
		suite.addTest(_createTestCase(sailStack, "range", "error001", false));
		suite.addTest(_createTestCase(sailStack, "type", "test001", true));
		suite.addTest(_createTestCase(sailStack, "type", "test002", true));
		suite.addTest(_createTestCase(sailStack, "type", "test003", true));
		suite.addTest(_createTestCase(sailStack, "type", "test004", true));
		suite.addTest(_createTestCase(sailStack, "type", "test005", true));
		suite.addTest(_createTestCase(sailStack, "type", "error001", false));
		suite.addTest(_createTestCase(sailStack, "type", "error002", false));

		return suite;
	}

	private static TestCase _createTestCase(Sail sailStack, String subdir, String testName,
			boolean isPositiveTest)
	{
